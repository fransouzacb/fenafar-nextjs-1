...
export default function AdminPage() {
  ...
  // Carregar dados reais da API de forma controlada
  const loadRealStats = async () => {
    if (hasLoadedStats.current || !user || !isAdmin() || isLoadingStats) {
      return
    }

    console.log('🔄 Carregando estatísticas reais...')
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.warn('⚠️ Erro na API, mantendo dados mock')
      }
    } catch (error) {
  ...
  useEffect(() => {
    ...
    } else if (user && isAdmin() && !hasLoadedStats.current) {
      // Carregar dados reais após 500ms para evitar loops
